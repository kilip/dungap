...
use Symfony\Component\Security\Http\AccessToken\Oidc\OidcTokenHandler as BaseTokenHandler;
    ...
    public function getUserBadgeFrom(string $accessToken): UserBadge
    {
        try {
            $oidcConfiguration = json_decode($this->cache->get('oidc.configuration', function (ItemInterface $item): string {
                ...
                $response = $this->securityAuthorizationClient->request('GET', $this->oidcIssuer.'.well-known/openid-configuration');

                return $response->getContent();
            }), true, 512, \JSON_THROW_ON_ERROR);
            $this->logger->notice('oidc configuration', $oidcConfiguration);
        } catch (\Throwable $e) {
